  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-3 rounded p-4" style={{ background: C.panel, border: `1px solid ${C.borderSoft}`, overflowX: "auto" }}>
        ...
        {/* Track MUSIK (3.3 swap musik + P4 volume/fade/upload) */}
        <div>
          ...
          {/* P4: expanded controls when music is ON */}
          {finishing.add_music && (
            <div className="flex items-center gap-4 flex-wrap rounded mt-1 px-3 py-2" style={{ background: C.panel, border: `1px solid ${C.borderSoft}` }}>
              {/* Volume */}
              <label className="flex items-center gap-1.5" style={{ fontFamily: F.mono, fontSize: 9.5, color: C.paperDim }}>
                VOL
                <input type="range" min={0} max={2} step={0.05} value={finishing.bgm_volume}
                  onChange={e => { pushHistory(); setFinishing({ ...finishing, bgm_volume: parseFloat(e.target.value) }); }}
                  style={{ width: 80, accentColor: C.music }} />
                <span style={{ fontFamily: F.mono, fontSize: 9, color: C.paper, width: 32, textAlign: "center" }}>{Math.round(finishing.bgm_volume * 100)}%</span>
              </label>
              {/* Fade in */}
              <label className="flex items-center gap-1.5" style={{ fontFamily: F.mono, fontSize: 9.5, color: C.paperDim }}>
                FADE IN
                <input type="number" min={0} max={10} step={0.5} value={finishing.bgm_fade_in}
                  onChange={e => { pushHistory(); setFinishing({ ...finishing, bgm_fade_in: parseFloat(e.target.value) || 0 }); }}
                  style={{ width: 44, fontFamily: F.mono, fontSize: 10, color: C.paper, background: C.panelRaised, border: `1px solid ${C.borderSoft}`, borderRadius: 3, padding: "2px 4px", outline: "none", textAlign: "center" }} />s
              </label>
              {/* Fade out */}
              <label className="flex items-center gap-1.5" style={{ fontFamily: F.mono, fontSize: 9.5, color: C.paperDim }}>
                FADE OUT
                <input type="number" min={0} max={10} step={0.5} value={finishing.bgm_fade_out}
                  onChange={e => { pushHistory(); setFinishing({ ...finishing, bgm_fade_out: parseFloat(e.target.value) || 0 }); }}
                  style={{ width: 44, fontFamily: F.mono, fontSize: 10, color: C.paper, background: C.panelRaised, border: `1px solid ${C.borderSoft}`, borderRadius: 3, padding: "2px 4px", outline: "none", textAlign: "center" }} />s
              </label>
              {/* Upload custom BGM */}
              <label className="flex items-center gap-1.5 cursor-pointer px-2 py-0.5 rounded"
              ...
              {finishing.bgm_custom_name && (
                <button onClick={() => { pushHistory(); setFinishing({ ...finishing, bgm_custom_path: "", bgm_custom_name: "" }); }}
